...
describe("resolveMaskSelection — T8 simple defaults + overrides", () => {
  ...
  it("user can mask fields on an object with NO detected sensitives (e.g. a app_ object)", () => {
    const sel = resolveMaskSelection(graph({ app_Employment_History__c: [] }), {
      app_Employment_History__c: ["Employer__c", "Supervisor_Name__c"],
    });
    expect(sel.get("app_Employment_History__c")?.get("Employer__c")).toBe("auto");
    expect(sel.get("app_Employment_History__c")?.get("Supervisor_Name__c")).toBe("auto");
  });
