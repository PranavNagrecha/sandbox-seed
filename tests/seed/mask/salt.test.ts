...
describe("masking salt store (T10)", () => {
  ...
  it("sanitizes aliases with spaces (real sandbox names like 'ExampleOrg FULL sandbox')", () => {
    expect(
      saltPath({ sourceAlias: "ExampleOrg FULL sandbox", targetAlias: "DevCaseInt", rootDir: root }),
    ).toBe(join(root, "id-maps", "ExampleOrg_FULL_sandbox__DevCaseInt.salt"));
  });
});
