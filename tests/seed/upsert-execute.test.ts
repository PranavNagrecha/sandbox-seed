 ...
 * Application"). Caught live against app_Application_Template__c.
 *
...
describe("runExecute — Name-field preservation on custom objects", () => {
  ...
  function mkCustomObjectDescribe(): SObjectDescribe {
    return {
      name: "app_Application_Template__c",
  ...
  function mkCustomLoadPlan(): LoadPlan {
    return {
      steps: [{ kind: "single", object: "app_Application_Template__c" }],
      excluded: [],
    };
  }
  ...
  it("includes Name in the insert body even when defaultedOnCreate=true", async () => {
    ...
    const desc = fakeDescribeClient({
      app_Application_Template__c: mkCustomObjectDescribe(),
    });
    ...
    await runExecute({
      ...
      targetDescribe: desc,
      graph: mkCustomGraph(),
      rootObject: "app_Application_Template__c",
      whereClause: "Id != null",
      finalObjectList: ["app_Application_Template__c"],
